...
### Added
- 配置页新增卡拉 OK 高亮颜色：支持 `#RRGGBB` / `#AARRGGBB`，留空时保持原有行为，跟随当前 Plasma 系统强调色；播放进度条同步使用该颜色

### Fixed
- 重新烘焙卡拉 OK 与跑马灯边缘淡出着色器，补齐桌面 GLSL 目标，修复部分 OpenGL 环境下歌词层不显示的问题
- 禁用长配置页输入框聚焦时的自动居中滚动，修复点击颜色或代理输入框时出现类似翻页动画的问题
- 自定义高亮色不再固定混入白色，所填颜色按原值渲染
- 配置页接收 Plasma 6 注入的 schema 默认属性，避免初始化告警

## [0.7.10] - 2026-08-13

### Added
- 单句歌词过长时启用跑马灯：文字超宽自动水平滚动（乒乓式，正向滚到行尾 → 停顿 → 反向滚回，无跳变）+ 14px 边缘渐变淡出遮罩，卡拉 OK 扫光与普通模式同步滚动；短句保持静止居中
- 恢复"两行·原词（当前 + 下一行）"显示模式（此前合并设置时遗漏）：当前行在上、下一行在下，字号按行高自动缩放

## [0.7.9] - 2026-08-13

### Changed
- 卡拉 OK 改为 ShaderEffect 扫光渲染：高亮/暗色沿 x 坐标平滑渐变混合，12px 柔和边缘 + 边缘微光，唱到的字呈"一半亮一半暗"，无生硬接缝；高亮向白色提亮，更醒目
- "歌词变体"与"显示行数"合并为一个设置：单行·原词/翻译/音译、两行·原词+翻译/原词+音译（原词在上、两行等高）、三行·原词（上下句预览）；行高与字号按面板高度自动计算，多行不互相挤压
- 双行模式改为锚点定位：卡拉 OK 接管主行时，翻译/音译副行固定在底部，不再与主行重叠

### Fixed
- KRC 逐字时间常早于下一行开始，导致扫光提前结束、整行长时间全亮：meta 新增行结束时间（line_end），QML 按行时长适度拉伸扫光（1.12~2.2 倍）
- 修复"显示模式"与"歌词变体"组合时的行数冲突（如三行 + 原词+翻译会叠成 4 行），两者已合并，冲突消除
